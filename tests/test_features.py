    ...
    def test_traffic_log(self, config, admin_url, admin_headers):
        ...
        for _ in range(2):
            ...
            for entry in data['log']['entries'][0:2]:
                assert entry['_serviceName'] == '%s:%s - Mock for Service1' % (SRV_8001_HOST, url_parsed1.port)
            ...
            if not admin_headers:
                ...
                for entry in data['log']['entries'][3:4]:
                    assert entry['_serviceName'] == '%s:%s - Mock for Service2' % (SRV_8002_HOST, url_parsed2.port)
                    assert datetime.fromisoformat(entry['startedDateTime']) > test_start_time
                    assert entry['time'] > 0
